# 橙知（OrangeMind）「考前 3 天冲刺」可点击演示稿

高保真级别的点击式网页原型说明，用于展示 Jry 在期末前 72 小时的完整冲刺路径与关键交互。视觉基调：白 + 橙 (#FF7A00)，简洁专业，带有温暖中性色点缀，吉祥物为成熟的“橙子小助理”。
...
### A. 首页 / 模块入口
- **结构**：
  - 顶栏：左侧 Logo + 模块名「橙知」；右上轻量吉祥物头像（微笑、举手动作），旁边小卡片显示今日建议（例：“建议先整理资料，15 分钟即可”）。
  - 主区域：左侧大标题“考前 3 天冲刺”，副标题“72 小时内完成应试路径”；右侧插画：橙子小助理递出任务树简图。
  - CTA：橙色主按钮“开始冲刺（只需 72 小时）”，副按钮“查看示例任务树”。
- **状态/动效**：
  - Hover：CTA 外发光橙色 glow，轻微上浮 4px；
  - Click：按钮轻微下沉并出现橙色涟漪，吉祥物点头。
- **点击**：主 CTA → B 资料上传页；副按钮 → 弹出示例任务树预览。

### B. 资料上传页
- **结构**：
  - 顶部进度条：显示“已上传 x / 6 项”，橙色填充。
  - 主区域左右分栏：
...
- **CTA**：橙色“生成任务树（系统思考）”，点击后按钮进入 loading 状态（橙色三点跳动，文案切换“生成中…”）。
- **点击**：提交成功 → D 个性化任务树；取消/关闭 → 返回 B。

### D. 个性化任务树（主界面）
- **结构**：
  - 顶栏：整体完成率环形图；倒计时“距考试 2 天 18:32”；右侧快捷操作（复习计划、模拟考入口）。
  - 主要区域：树状/矩阵卡片，每个节点=考点；卡片包含标题、难度点位、估时、完成环；颜色：未做浅灰、进行中淡橙、完成亮橙打勾。
...
- **示例模板（默认展示）**：
  ...
  3. 行化简检验特征向量维度。
- **完成按钮**：右下角灰色“完成并标记”，Hover 转橙，点击 → 节点标为完成并回写到任务树。
...
- **结构**：
  ...
  - 分数区：最终成绩、预计区间（保守/乐观）；
  - 弱点摘要卡 + 行动建议；
  - CTA：橙色“复习错题本”、次要“导出考前小抄（仅复习要点）”，第三按钮“再次模拟”。
...
- **上传**：卡片显示橙色 loading 条；成功后轻微缩放 + 绿勾；失败抖动一次。
- **任务树节点完成**：节点闪烁 0.3s → 橙色打勾 → 顶部 XP 徽章下落并消失；进度条平滑增长。
- **Chat Sidebar**：菜单触发后展开，自右侧滑入；AI 回复逐字渐显；支持继续追问。
- **批改返回**：上传后显示橙色点阵 loading；批改完成卡片自下弹出；正确时短暂闪亮，错误时红色波纹强调错行。
- **错题本安排**：拖拽题目到时间线时，时间块变橙色高亮；放开时伴随轻微震动反馈。
...
- **按钮规范**：
  - 主按钮：橙底白字，圆角 10px，左右内边距 16–20px，hover 外发光；
  - 次按钮：白底橙边，文字橙；
  - icon-only：圆形浅灰背景，橙色线条图标。
